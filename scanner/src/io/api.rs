...
use sdr::FreqBlock;
use shared::{ConnectionType, DisplayInfo, External};
...
pub async fn start(in_tx: Sender<Input>, out_tx: BroadcastSender<Output>, realtime_rx: WatchReceiver<FreqBlock>, initial_display_info: DisplayInfo) {
    tokio::task::spawn(async move {
        ...
        for stream in listener.incoming().flatten() {
            let (in_tx, out_rx, realtime_rx, current_data) = (in_tx.clone(), out_tx.subscribe(), realtime_rx.clone(), initial_display_info.clone());
            let _ = tokio::task::spawn(async move { handle_client(stream, in_tx, out_rx, realtime_rx, current_data).await}).await;
        }
    });
}

async fn handle_client(mut stream: UnixStream, in_tx: Sender<Input>, out_rx: BroadcastReceiver<Output>, realtime_rx: WatchReceiver<FreqBlock>, initial_display_info: DisplayInfo) {
    ...
    if let External::Connection(conn_type) = bincode::decode_from_std_read(&mut stream, config).unwrap() {
        ...
        match conn_type {
            ConnectionType::Display => {handle_display_client(stream, config, in_tx, out_rx, realtime_rx, initial_display_info).await;},
