...
impl IOManager {
    ...
    pub async fn start(
        ...
        tokio::spawn(self.start_listener(
            external_tx,
            realtime_rx,
            internal_tx,
    ...
    async fn start_listener(
        ...
        let listener = UnixListener::bind("/tmp/sdrscanner").expect("Failed to bind Unix socket");
        ...
        loop {
            ...
            match listener.accept().await {
                Ok((mut stream, _)) => {
                    ...
                    let new_count =
                        client_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed) + 1;
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Err(e) = send_to_client(&mut stream, &initial_display_info).await {
                            error!(
                                "Failed to send initial display info to client {}: {}",
                                client_id, e
                            );
                            let new_count =
                                client_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed) - 1;
                            info!(
                                "Client {} disconnected during initial send, client count decremented to {}",
                                client_id, new_count
                            );
                            return;
                        }
